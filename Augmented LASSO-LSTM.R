...
# Library
#########
# These should cover all needs to implement.
# It is important that the keras is installed correctly 
# Look into tensorflow to make this model work
...
# LASSO
#######
# This part covers the LASSO section. We apply the LASSO feature selection
# from the previous model, we know that the feature selection could have been
# done on the MBB dataset, but since the original data capture all 
# relationships in the real world, LASSO on MBB  would only introduce more
# noise. Use the feature selection from previous methods. 

######
# Data
######

low_dat <- fread('C:/Users/tormo/OneDrive/Skole/Masteroppgave/Data/aug_low_dat.csv')
# low_dat$test_CPI <- low_dat$CPIAUCSL
# View(low_dat)

#Here you can see features for a strict feature selection.
...
split_point <- 0.8
split_index <- 2796 #floor(nrow(low_dat) * split_point) #splitindex
forecast_horizon <- 3
...
# X_train <- as.matrix(train_dat[, -which(names(train_dat) == 'date'), with = FALSE])
# X_train <- select(train_dat, -date) #$date <- NULL
# X_train$CPIAUCSL <- NULL
# X_train$cpi
...
# Here you can see your coefs if interested in seeing LASSO features here.
# Adjust LASSO above based on preference. 
...
# 1,3,6 MND
###########
# These features could be applied for a shorter forecast horizon,
# and are what we genreally recommend. 
...
# 12,24,48 MND
##############
# These should be better for longer forecast horizons. 
# Choose your level of strictness of lambda for more or less features.
...
sequences <- create_sequences(dat, y, 48, forecast_horizon)
...
# Running the model
###################
# Here you can adjust the model. Set hyperparameters 
# based on results from validation 
dropout_rate <- 0.1
model_units <- length(important_variables)
# Define and compile the model
model <- keras_model_sequential() %>%
  layer_lstm(units = model_units, input_shape = c(48, n_features), return_sequences = TRUE) %>%
...
# you can adjust the learning rate as adam or choose your self.

model %>% compile(
  optimizer = 'adam',#optimizer_adam(lr = 0.001),#'adam',
  loss = 'mean_squared_error',
  metrics = 'mean_absolute_error'
)

# More hyperparamters for you to choose. This is the part when
# running that takes a long time. 
# Train the model
callback_early_stopping <- callback_early_stopping(monitor = "val_loss", patience = 30, restore_best_weights = TRUE)
history <- model %>% fit(
  X_train, Y_train,
  epochs = 200,
  batch_size = 128,
...
# RMSE
######
# you can save your rmse results if your interested. 
